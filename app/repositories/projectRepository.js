...
const addUserToProject=async(data)=>{
    ...
    let project=await (await db.query("select * from projects where id="+projectId)).rows[0]
    let user=await (await db.query("select * from users where user_name=$1",[userName])).rows[0]
